...
local raw_road_tile_built = function(event)
  ...
  depot_common.on_road_tile_placed(event.surface_index, positions)
      ...
      if event.player_index then
        ...
        if player then
          for name, count in pairs (refund_items) do
            player.insert({name = event.item.name, count = count})
...
local script_raised_set_tiles = function(event)
  ...
  for k, tile in pairs (event.tiles) do
    if is_road_tile(tile.name) then
      ...
      road_positions[#road_positions + 1] = tile.position
  ...
  if #road_positions > 0 then
    depot_common.on_road_tile_placed(event.surface_index, road_positions)
  end
